...
function onSlideTransitionEnd(){
    ...
    if(state.currentSlideIndex === slideItems.length -2){
        setVisibleSlide({index: 2, animate: false}); //para ficar visivel a animacao e tirar o infinito, colocar true
    }
    if(state.currentSlideIndex === 1){
        setVisibleSlide({index:slideItems.length -3, animate: false}); //para ficar visivel a animacao e tirar o infinito, colocar true
    }
}
...
function setListeners(){
    ...
    slideList.addEventListener('transitionend',onSlideTransitionEnd);
